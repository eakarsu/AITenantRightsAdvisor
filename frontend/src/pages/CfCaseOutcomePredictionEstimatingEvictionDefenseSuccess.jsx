...
// Feature: Case outcome prediction estimating eviction defense success probability
...
export default function CfCaseOutcomePredictionEstimatingEvictionDefenseSuccess() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-case-outcome-prediction-estimating-eviction-defense-success-probability/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-case-outcome-prediction-estimating-eviction-defense-success-probability', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-case-outcome-prediction-estimating-eviction-defense-success-probability/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Case outcome prediction estimating eviction defense success probability</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITenantRightsAdvisor</p>
      </div>
