...
// Feature: Document assembly auto-filling forms with tenant data
...
export default function CfDocumentAssemblyAutoFillingFormsWithTenant() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-document-assembly-auto-filling-forms-with-tenant-data/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-document-assembly-auto-filling-forms-with-tenant-data', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-document-assembly-auto-filling-forms-with-tenant-data/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Document assembly auto-filling forms with tenant data</h1>
