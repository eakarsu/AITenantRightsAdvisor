...
export default function GapNoConversationalTenantRightsChatbotBeyondCore() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-conversational-tenant-rights-chatbot-beyond-core-ai-endpoints/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-conversational-tenant-rights-chatbot-beyond-core-ai-endpoints', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-conversational-tenant-rights-chatbot-beyond-core-ai-endpoints/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No conversational tenant-rights chatbot beyond core AI endpoints</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AITenantRightsAdvisor</p>
      </div>
