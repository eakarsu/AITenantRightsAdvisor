...
// Feature: Local law adaptation customizing advice by state/city
...
export default function CfLocalLawAdaptationCustomizingAdviceByState() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-local-law-adaptation-customizing-advice-by-state-city/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-local-law-adaptation-customizing-advice-by-state-city', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-local-law-adaptation-customizing-advice-by-state-city/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Local law adaptation customizing advice by state/city</h1>
