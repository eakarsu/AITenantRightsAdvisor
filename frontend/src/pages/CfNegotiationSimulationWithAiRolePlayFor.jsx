...
// Feature: Negotiation simulation with AI role-play for mediation practice
...
export default function CfNegotiationSimulationWithAiRolePlayFor() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-negotiation-simulation-with-ai-role-play-for-mediation-practice/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-negotiation-simulation-with-ai-role-play-for-mediation-practice', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-negotiation-simulation-with-ai-role-play-for-mediation-practice/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Negotiation simulation with AI role-play for mediation practice</h1>
