...
// Feature: Deadline tracker with notifications for notice/response windows
...
export default function CfDeadlineTrackerWithNotificationsForNoticeResponse() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-deadline-tracker-with-notifications-for-notice-response-windows/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-deadline-tracker-with-notifications-for-notice-response-windows', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-deadline-tracker-with-notifications-for-notice-response-windows/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Deadline tracker with notifications for notice/response windows</h1>
