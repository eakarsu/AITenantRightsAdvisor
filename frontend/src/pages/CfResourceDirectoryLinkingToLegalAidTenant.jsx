...
export default function CfResourceDirectoryLinkingToLegalAidTenant() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-resource-directory-linking-to-legal-aid-tenant-unions/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-resource-directory-linking-to-legal-aid-tenant-unions', project: 'AITenantRightsAdvisor' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-resource-directory-linking-to-legal-aid-tenant-unions/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Resource directory linking to legal aid, tenant unions, housing advocates</h1>
