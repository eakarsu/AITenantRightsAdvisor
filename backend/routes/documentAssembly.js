...
// Document assembly auto-filling forms with tenant data
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'document-assembly', payload, result);
    res.json({ success: true, feature: 'document-assembly', ...result });
  } catch (err) {
    console.error('document-assembly analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Document assembly auto-filling forms with tenant data. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'document-assembly', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'document-assembly', title: 'Document assembly auto-filling forms with tenant data', project: 'AITenantRightsAdvisor' });
});
