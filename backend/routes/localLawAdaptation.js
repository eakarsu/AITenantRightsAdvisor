...
// Local law adaptation customizing advice by state/city
// Feature: local-law-adaptation
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'local-law-adaptation', payload, result);
    res.json({ success: true, feature: 'local-law-adaptation', ...result });
  } catch (err) {
    console.error('local-law-adaptation analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Local law adaptation customizing advice by state/city. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'local-law-adaptation', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'local-law-adaptation', title: 'Local law adaptation customizing advice by state/city', project: 'AITenantRightsAdvisor' });
});
