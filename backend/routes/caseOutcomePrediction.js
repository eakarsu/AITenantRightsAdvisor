...
// Case outcome prediction estimating eviction defense success probability
// Feature: case-outcome-prediction
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'case-outcome-prediction', payload, result);
    res.json({ success: true, feature: 'case-outcome-prediction', ...result });
  } catch (err) {
    console.error('case-outcome-prediction analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Case outcome prediction estimating eviction defense success probability. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'case-outcome-prediction', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'case-outcome-prediction', title: 'Case outcome prediction estimating eviction defense success probability', project: 'AITenantRightsAdvisor' });
});
