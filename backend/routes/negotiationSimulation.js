...
// Negotiation simulation with AI role-play for mediation practice
// Feature: negotiation-simulation
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'negotiation-simulation', payload, result);
    res.json({ success: true, feature: 'negotiation-simulation', ...result });
  } catch (err) {
    console.error('negotiation-simulation analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Negotiation simulation with AI role-play for mediation practice. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'negotiation-simulation', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'negotiation-simulation', title: 'Negotiation simulation with AI role-play for mediation practice', project: 'AITenantRightsAdvisor' });
});
