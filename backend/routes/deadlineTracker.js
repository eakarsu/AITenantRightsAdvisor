...
// Deadline tracker with notifications for notice/response windows
...
router.post('/analyze', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    await persist(uid, 'deadline-tracker', payload, result);
    res.json({ success: true, feature: 'deadline-tracker', ...result });
  } catch (err) {
    console.error('deadline-tracker analyze error:', err.message);
...
router.post('/batch', authMiddleware, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Deadline tracker with notifications for notice/response windows. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'deadline-tracker', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', authMiddleware, (req, res) => {
  res.json({ feature: 'deadline-tracker', title: 'Deadline tracker with notifications for notice/response windows', project: 'AITenantRightsAdvisor' });
});
